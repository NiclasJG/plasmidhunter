...
export default (config) => {
    function init(selection) {
        ...
        let timeScale = d3
            .scaleTime()
            .domain([d3.min(events.map((e) => e.start)), d3.max(events.map((e) => e.end))])
            .range([0, width])

        let graph = svg.append('g').classed('graph', true).attr('transform', `translate(${margin.left},${margin.top})`)

        let view = graph.append('g').classed('view', true)
